...
public class MainScreen extends LoginComponent {
	...
	public void render(Client client) {
		...
		client.boldText.drawText(true, 160, 0xCB9535, Utility.formatName(client.myUsername) + ((client.loginScreenCursorPos == 0) & (Client.tick % 40 < 20) ? "|" : ""), 295);
		client.boldText.drawText(true, 160, 0xCB9535, StringUtils.toAsterisks(client.myPassword) + ((client.loginScreenCursorPos == 1) & (Client.tick % 40 < 20) ? "|" : ""), 345);
		...
		for (int i = 0; i < ProfileManager.MAX_PROFILES; i++, profileY += 65) {
			...
			profile.drawProfileHead(489, profileY);
			client.boldText.drawCenteredText(0xCB9535, 585, Utility.formatName(profile.getUsername()), profileY + 29, true);
		}
	}
